...
public class LightLoaderCallbacks<T> implements LoaderCallbacks<LightResponse<T>> {
    ...
    public LightLoaderCallbacks(Context context, LightRequestCallbacks<T> apiCallbacks, LightRequest<T> request) {
    ...
    public static void handleRequestServerErrorMessage(final String errorTitle, final String errorMessage) {
        // TODO
    }
    ...
    public Loader<LightResponse<T>> onCreateLoader(int loadId, Bundle bundle) {
        return new LightLoader<LightResponse<T>>(mContext);
    }
    ...
    public void onLoadFinished(Loader<LightResponse<T>> loader, LightResponse<T> response) {
        ...
        if (response.getMetaCode() != LightResponseCode.META_CODE_OK) {
            ...
            if (mRequest.shouldShowAlertForRequest(response)) {
                handleRequestServerErrorMessage(response.getErrorTitle(), response.getErrorMessage());
            }
